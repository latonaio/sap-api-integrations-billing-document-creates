...
import (
	sap_api_caller "sap-api-integrations-billing-document-creates/SAP_API_Caller"
	sap_api_input_reader "sap-api-integrations-billing-document-creates/SAP_API_Input_Reader"
	"sap-api-integrations-billing-document-creates/config"
...
func main() {
	...
	inputSDC          := fr.ReadSDC("./Inputs/SDC_Billing_Document_sample.json")
	accepter          := getAccepter(inputSDC)
	header            := inputSDC.ConvertToHeader()
	item              := inputSDC.ConvertToItem()

	caller.AsyncPostBillingDocument(
		header,
		item,
		accepter,
	)
